"""
MANAPRINT — Générateur OAOA (format A4)
12 cartes par feuille (3 colonnes × 4 rangées).
Chaque carte : 2 colonnes O (16-30) et A (61-75), en-têtes avec plages,
3 numéros par colonne TRIÉS du plus petit au plus grand.
Bande droite réservée au QR de sécurité (aucun chiffre dérangé).
...
RANGES = [(16, 30), (61, 75)]
LETTRES = ["O", "A"]
QR_STRIP = 17 * mm  # bande droite réservée au QR
FOOT_H = 3 * mm
BANDEAU_H = 2.6 * mm
HDR_H = 3.4 * mm
GRID_N = 2  # 2 colonnes (O, A) x 3 rangées
ROWS_N = 3


def _gen_carte():
    """3 numéros TRIÉS par colonne : dict {(ligne, col): n}."""
    carte = {}
    for ci, (lo, hi) in enumerate(RANGES):
        nums = sorted(random.sample(range(lo, hi + 1), ROWS_N))
...
def _dessiner_carte(c, x0, y0, donnees, couleur_hex, serie, encre,
    ...
    # En-tête : lettres O / A + leurs plages
    hdr_y = y0 + CARD_H - BANDEAU_H - HDR_H - 2.0 * mm
    for i, lettre in enumerate(LETTRES):
        cx = x0 + (i + 0.5) * cell_w
        c.setFillColor(col); c.setFont("Helvetica-Bold", 8)
        c.drawCentredString(cx, hdr_y + 2.2 * mm, lettre)
        lo, hi = RANGES[i]
        c.setFillColor(GREY); c.setFont("Helvetica", 4)
        c.drawCentredString(cx, hdr_y + 0.4 * mm, f"{lo} - {hi}")
    c.setStrokeColor(col); c.setLineWidth(0.4)
    c.line(x0, hdr_y, x0 + grid_w, hdr_y)

    # Grille 3×3
    grid_top = hdr_y
    zone_h = grid_top - (y0 + FOOT_H)
    cell_h = zone_h / ROWS_N
    ...
    taille = 33  # gros chiffres bien lisibles (2 colonnes = grandes cases)
    for ri in range(ROWS_N):
    ...
    c.drawRightString(x0 + grid_w - 1.5 * mm, y0 + 1.3 * mm, f"{serie:06d}")
    if telephone:
        c.drawString(x0 + 12 * mm, y0 + 1.3 * mm, f"\u00b7 Resp. {telephone}")

    # 🎯 QR dans la bande droite réservée (aucun chiffre dérangé)
    if _sec and evenement_id:
        try:
            _q = 12.5 * mm
            _xq = x0 + CARD_W - _q - 2.2 * mm
            _yq = y0 + (CARD_H - _q - 3.6 * mm) / 2 + 3.6 * mm
...
def generer_pdf(nb_cartes=12, serie_start=1, theme="", couleur=True,
    ...
    for _ in range(nb_pages):
        ...
        titre_aff = titre_jeu if titre_jeu else "OAOA"
        ligne2 = titre_aff
...
if __name__ == "__main__":
    ...
    with open("test_oaoa.pdf", "wb") as f:
        f.write(pdf.read())
    print("OAOA g\u00e9n\u00e9r\u00e9")
