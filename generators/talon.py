# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur TALON (format A4)

⚽ NÉ LE 15/08 (sceau Maeva) — LE CINQUIÈME JEU DE RANIHEI, rien qu'à elle.
Un ballon de football, six panneaux, un numéro dans chacun.

RÈGLE (sceau Maeva 15/08, relevée sur sa maquette 7·23·34·52·75·83) :
six numéros, un par panneau —
  1 · panneau de GAUCHE      : 1-15
  2 · panneau du HAUT        : 16-30
  3 · panneau de DROITE      : 31-45
  4 · panneau BAS-DROITE     : 46-60
  5 · panneau BAS-GAUCHE     : 61-75
  6 · le PENTAGONE du CENTRE : 76-90
⚠️ Les six plages ne se chevauchent pas : les numéros sont donc
toujours différents.
...
import os as _os2
from reportlab.pdfbase.pdfmetrics import stringWidth as _lg_ta
...
_IMAGE_TALON = _choisir_image("talon_ballon", _RATIO_TALON)
...
GUTTER_X = 4 * mm
GUTTER_Y = 3 * mm
# ⚠️⚠️ LE DESSIN EST CARRÉ : la hauteur commande ici, sinon 4 rangées de
# 95 mm ne tiendraient jamais sur la feuille. La largeur suit, on centre.
# ⭐ 15/08 : DOUZE grilles par feuille (3 colonnes x 4 rangées) — le carré
# passe de 67,5 à 62 mm, à peine plus petit, et Maeva en tire 50 % de plus.
COLS_PAGE = 3
ROWS_PAGE = 4
# ⚠️⚠️ LE BALLON EST CARRÉ : la carte doit l'être aussi, sinon il devient
# un œuf. On prend donc le PLUS PETIT des deux côtés disponibles, et on
# centre le bloc dans les deux sens.
...
CARD_W = min(_DISPO_W, _DISPO_H * _RATIO_TALON)
CARD_H = CARD_W / _RATIO_TALON
...
def _gen_carte(rng):
    """⚽ Six numéros, un par panneau, dans sa plage. Ils sont forcément
...
def _dessiner_carte(c, x0, y0, nums, couleur_hex, serie, telephone="",
    ...
    _px = x0 + (CARD_W - _pw) / 2
    _py = y0 + 0.3 * mm
    if _os2.path.exists(_IMAGE_TALON):
        try:
            c.drawImage(_IMAGE_TALON, _px, _py, _pw, _ph, mask="auto")
        except Exception:
            pass
    ...
    # ═══ LA SÉRIE, discrète en haut à droite ═══
    c.setFillColor(gris_ch)
    _tb = 8.0
    _bl = "N\u00b0 %05d" % serie
