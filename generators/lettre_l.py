# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur LETTRE L (format A4)
6 cartes par feuille A4 (2 colonnes × 3 rangées).
Chaque carte : 9 numéros disposés en GRAND L (fidèle au modèle) :
  · bras VERTICAL : 5 numéros 1-15, triés vers le bas (4 cases + le coin)
  · le PIED : 4 numéros — un en 16-30, un en 31-45, un en 46-60, un en 61-75
Au centre : le PERSONNAGE L aux yeux malicieux (dessiné main),
...
PAGE_W, PAGE_H = A4
PLAGE_GAUCHE = (1, 15)     # 5 numéros (bras vertical + coin)
PIED = [(16, 30), (31, 45), (46, 60), (61, 75)]   # 1 numéro chacun
...
def _gen_carte(rng):
    """9 numéros : 5 dans le bras vertical (1-15) triés, 4 au pied (un par famille)."""
    gauche = sorted(rng.sample(range(PLAGE_GAUCHE[0], PLAGE_GAUCHE[1] + 1), 5))
    pied = [rng.randint(pmin, pmax) for pmin, pmax in PIED]
    return gauche, pied


def _personnage_l(c, cx, cy, h, col):
    """Le personnage L aux yeux malicieux (fidèle au modèle)."""
    w = h * 0.62
    ep = h * 0.17          # l'épaisseur du corps du L
    c.saveState()
    c.setStrokeColor(col); c.setLineWidth(1.1)
    # le corps : un L dodu au pied arrondi
    p = c.beginPath()
    p.moveTo(cx - w / 2, cy + h / 2)
    p.lineTo(cx - w / 2, cy - h / 2)
    p.lineTo(cx + w / 2 - ep * 0.4, cy - h / 2)
    p.arcTo(cx + w / 2 - ep * 1.4, cy - h / 2, cx + w / 2, cy - h / 2 + ep * 1.6,
            startAng=270, extent=180)
    p.lineTo(cx - w / 2 + ep, cy - h / 2 + ep)
    p.lineTo(cx - w / 2 + ep, cy + h / 2)
    p.close()
    c.drawPath(p, stroke=1, fill=0)
    # les deux yeux malicieux au sommet du bras
    for i, dx in enumerate((-ep * 0.28, ep * 0.5)):
        ex, ey = cx - w / 2 + ep / 2 + dx, cy + h / 2 + h * 0.05
        c.setFillColor(colors.white); c.setStrokeColor(col); c.setLineWidth(0.9)
        c.circle(ex, ey, h * 0.115, stroke=1, fill=1)
        c.setFillColor(col)
        c.circle(ex + h * 0.03, ey + h * 0.025, h * 0.045, stroke=0, fill=1)
    c.restoreState()
...
def _dessiner_carte(c, x0, y0, nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id=""):
    ...
    # Bras vertical (4 cases) — triés vers le bas
    for ri in range(4):
        case(x0, y0 + CARD_H - (ri + 1) * bras_h, cell_w, bras_h, gauche[ri])
    # Le pied du L : le coin + les 4 familles
    bas = [gauche[4]] + pied
    for i, val in enumerate(bas):
        case(x0 + i * cell_w, y0, cell_w, BAS_H, val)

    # L'étage central, de haut en bas : le personnage L 😄, la signature,
...
def generer_pdf(nb_cartes=6, serie_start=1, theme="", couleur=True,
    ...
    rng = random.Random(934900 + int(serie_start))
...
if __name__ == "__main__":
    ...
    with open("test_lettre_l.pdf", "wb") as f:
        f.write(pdf.read())
    print("LETTRE L généré")
