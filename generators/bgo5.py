# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur BGO 5 BOULES (format A4)
12 cartes par feuille A4 (3 colonnes × 4 rangées).
Chaque carte : en-tête B | G | O puis grille complète 3×3 — 5 boules
seulement (fidèle au modèle) :
  B = 1-15 (×2) et O = 61-75 (×2) sur les rangées 1 et 2, triés,
  le G trône SEUL en BAS au centre (46-60).
Le QR de vérification habite la case vide du CŒUR de la grille.
Pied de carte : « N° SERIE | 030001 ». Le caller saute le 16-45 !
...
PAGE_W, PAGE_H = A4
LETTRES = "BGO"
# (min, max, rangées occupées) par lettre — le G seul en bas,
# B et O sur les rangées 1 et 2 (fidèle au modèle)
COLONNES = [(1, 15, (0, 1)), (46, 60, (2,)), (61, 75, (0, 1))]
...
def _dessiner_carte(c, x0, y0, cols_nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id="", motif=""):
    ...
    signature = "BGO 5 boules by TUKEA"
    if titre_jeu and "BGO" not in titre_jeu.strip().upper():
...
def generer_pdf(nb_cartes=12, serie_start=1, theme="", couleur=True,
    ...
    rng = random.Random(939100 + int(serie_start))
