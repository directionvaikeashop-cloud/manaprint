...
def _dessiner_carte(c, x0, y0, groupes, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id=""):
    ...
    for gi, nums in enumerate(groupes):
        ...
        if _sec:  # chiffres "billet de banque" remplis de microtexte
            _sec.chiffre_micro(c, n_cercle, cx1, haut_y - 4.2, 19, gris_ch, police_ch)
        else:
            c.setFillColor(gris_ch); c.setFont(police_ch, 19)
            c.drawCentredString(cx1, haut_y - 4.2, str(n_cercle))
        # 2e numéro : à droite du cercle
        cx2 = gx + cell_w * 0.72
        if _sec:
            _sec.chiffre_micro(c, n_grand, cx2, haut_y - 4.2, 19, gris_ch, police_ch)
        else:
            c.setFillColor(gris_ch); c.setFont(police_ch, 19)
            c.drawCentredString(cx2, haut_y - 4.2, str(n_grand))
        # 3e numéro : plus petit, centré dessous
        if _sec:
            _sec.chiffre_micro(c, n_petit, gx + cell_w / 2, bas_y, 15, gris_ch, police_ch)
        else:
            c.setFillColor(gris_ch); c.setFont(police_ch, 15)
