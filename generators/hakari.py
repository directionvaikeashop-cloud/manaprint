...
def _charger_cocotier():
    """L'illustration du cocotier (licence Freepik), pâlie en filigrane.
    Anti-panne : si le fichier manque, retour au palmier vectoriel maison."""
    global _COCOTIER_IMG
    if _COCOTIER_IMG is not None:
        return _COCOTIER_IMG
    try:
        from PIL import Image as _Image
        chemin = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "cocotier_hakari.png")
...
def _dessiner_fond(c, x0, y0, w, h):
    """Le cocotier HAKARI : l'illustration sous licence en filigrane —
    et le palmier vectoriel maison en roue de secours si l'image manque."""
    img = _charger_cocotier()
    if img:
        from reportlab.lib.utils import ImageReader
        iw, ih = img.size
        zone_w, zone_h = w * 0.80, h * 0.72
        ...
        return
    # ── repli : le palmier vectoriel maison ──
    bx, by = x0 + w * 0.52, y0 + 3 * mm
    c.setStrokeColor(PALE2); c.setLineWidth(2.2); c.setLineCap(1)
    p = c.beginPath()
    p.moveTo(bx - w * 0.06, by)
    p.curveTo(bx - w * 0.02, by + h * 0.3, bx + w * 0.02, by + h * 0.5, bx, by + h * 0.55)
    c.drawPath(p, stroke=1, fill=0)
    c.setLineWidth(1.0)
    for ang in (18, 55, 95, 135, 168, -12):
        c.saveState(); c.translate(bx, by + h * 0.55)
        c.rotate(ang)
        # la nervure
        pf = c.beginPath()
        pf.moveTo(0, 0)
        pf.curveTo(w * 0.10, h * 0.06, w * 0.22, h * 0.05, w * 0.30, -h * 0.02)
        c.drawPath(pf, stroke=1, fill=0)
        # la palme dentelée (zigzag sous la nervure)
        pz = c.beginPath()
        pz.moveTo(w * 0.02, -h * 0.004)
        for k in range(6):
            t_ = (k + 1) / 6.0
            xk = w * (0.02 + 0.27 * t_)
            yk = h * (0.055 * t_ - 0.02 * t_ * t_)
            pz.lineTo(xk - w * 0.02, yk - h * 0.045)
            pz.lineTo(xk, yk - h * 0.006)
        c.drawPath(pz, stroke=1, fill=0)
        c.restoreState()
...
def _dessiner_carte(c, x0, y0, nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id=""):
    ...
    for i, (px, py) in enumerate(POSITIONS[:len(nums)]):
        ...
        cy = y0 + CARD_H * py
        if i == 0:
            c.setStrokeColor(colors.Color(0.55, 0.55, 0.55)); c.setLineWidth(1.1); c.setFillColor(colors.white)
            c.circle(cx, cy - TAILLE_CHIFFRE * 0.14, TAILLE_CHIFFRE * 0.74, stroke=1, fill=1)
        if _sec:
