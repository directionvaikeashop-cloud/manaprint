# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur HUNTER (format A4)

🎯 NÉ LE 15/08 (sceau Maeva) — LE JEU DE RANIHEI, rien qu'à elle.
Un ticket allongé, six cercles alignés, chacun coiffé de SA PAIRE DE
LETTRES : BO · IG · NB · GI · OB · ⭐

RÈGLE (sceau Maeva 15/08) : six numéros, TOUS DIFFÉRENTS. Chaque boule
tire dans l'UNE OU L'AUTRE de ses deux plages —
  1 · BO : B 1-15  ou  O 61-75
  2 · IG : I 16-30 ou  G 46-60
  3 · NB : N 31-45 ou  B 1-15
  4 · GI : G 46-60 ou  I 16-30
  5 · OB : O 61-75 ou  B 1-15
  6 · ⭐ : 76-90 (l'ÉTOILE, la plage réservée)
⚠️ Le sac du crieur va donc de 1 à 90.

8 tickets par feuille A4 (2 colonnes × 4 rangées).
"""
import io
...
_RATIO_TICKET = 3.1915
CERCLES = [[0.0958, 0.5072], [0.258, 0.5071], [0.42, 0.5071], [0.5815, 0.5071], [0.7427, 0.5071], [0.9027, 0.5071]]
LARG_CERCLE = 0.0899
HAUT_CERCLE = 0.2872
...
_IMAGE_TICKET = _choisir_image("hunter_languette", _RATIO_TICKET)
...
GUTTER_X = 4 * mm
GUTTER_Y = 4 * mm
# ⭐⭐ 15/08 : EN LANGUETTES, comme OHANA 75 · 8 boules — cinq bandes
# empilées, découpe en ligne droite.
# ⚠️⚠️ On RESPECTE LE RATIO du dessin (3,19) : la hauteur commande, la
# largeur suit, et la languette est centrée. Sinon le dessin s'aplatit.
COLS_PAGE = 1
ROWS_PAGE = 5
CARD_H = (PAGE_H - MARGIN_TOP - MARGIN_BOT - (ROWS_PAGE - 1) * GUTTER_Y) / ROWS_PAGE
CARD_W = min(PAGE_W - 2 * MARGIN_X, CARD_H * _RATIO_TICKET)
MARGE_G = (PAGE_W - CARD_W) / 2      # ce qu'il faut pour la centrer
...
def _dessiner_ticket(c, x0, y0, nums, couleur_hex, serie, telephone="",
    ...
    _t_num = 34.0
    while _t_num > 6 and (_lg_hu("88", _POLICE_NUM, _t_num) > _lg_c * 0.52
    ...
    for _k, _paire in enumerate(nums[:6]):
        ...
        for _i, _n in enumerate(_paire[:2]):
            _nx = _cx - _dx if _i == 0 else _cx + _dx
            _ny = (_cy + _dy) if _i == 0 else (_cy - _dy)
            c.setFillColor(gris_ch)
            c.setFont(_POLICE_NUM, _t_num)
            c.drawCentredString(_nx, _ny - _t_num * 0.34, str(_n))

    # ═══ LA SÉRIE, dans le pied du ticket ═══
    c.setFillColor(gris_ch)
    _tb = 9.0
    _bl = "%05d" % serie
    while _tb > 3.2 and _lg_hu(_bl, "Helvetica-Bold", _tb) > _pw * 0.13:
        _tb -= 0.25
    c.setFont("Helvetica-Bold", _tb)
    c.drawCentredString(_px + _pw * 0.838, _py + _ph * 0.098, _bl)
