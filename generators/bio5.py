# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur BIO 5 BOULES (format A4)
12 cartes par feuille A4 (3 colonnes × 4 rangées).
Chaque carte : en-tête B | I | O puis grille complète 3×3 — 5 numéros
seulement (fidèle au modèle) :
  le I trône SEUL en haut au centre (16-30),
  B = 1-15 (×2) et O = 61-75 (×2) sur les rangées 2 et 3, triés.
Le QR de vérification habite la case vide du CŒUR de la grille.
Pied de carte : « N° SERIE | 030001 ». Le caller saute le 31-60 !
...
PAGE_W, PAGE_H = A4
LETTRES = "BIO"
# (min, max, rangées occupées) par lettre — le I seul en haut,
# B et O sur les rangées 2 et 3 (fidèle au modèle)
COLONNES = [(1, 15, (1, 2)), (16, 30, (0,)), (61, 75, (1, 2))]
...
def _gen_carte(rng):
    """5 numéros : 1 pour I, 2 pour B et O, triés vers le bas."""
...
def _dessiner_carte(c, x0, y0, cols_nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id="", motif=""):
    ...
    signature = "BIO 5 boules by TUKEA"
    if titre_jeu and "BIO" not in titre_jeu.strip().upper():
    ...
    # QR de vérification — logé dans la case vide du CENTRE (la signature TUKEA)
...
def generer_pdf(nb_cartes=12, serie_start=1, theme="", couleur=True,
    ...
    rng = random.Random(936400 + int(serie_start))
...
if __name__ == "__main__":
    ...
    with open("test_bio.pdf", "wb") as f:
        f.write(pdf.read())
    print("BIO 5 boules généré")
