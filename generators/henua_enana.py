# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur HENUA ENANA (format A4)
12 cartes/page — archipel dessiné maison avec ses îles, 6 grands numéros.
...
def _charger_marquises():
    """L'illustration des Marquises (licence Freepik, fournie par Maeva),
    pâlie en filigrane. Anti-panne : archipel vectoriel maison en secours."""
    global _MARQUISES_IMG
    if _MARQUISES_IMG is not None:
        return _MARQUISES_IMG
    try:
        from PIL import Image as _Image
        chemin = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "marquises_henua_enana.png")
...
NB_NUMS = 7
POSITIONS = [(0.15, 0.74), (0.14, 0.48), (0.16, 0.20), (0.84, 0.76), (0.84, 0.52), (0.84, 0.30), (0.45, 0.16)]
TAILLE_CHIFFRE = 32


def _dessiner_fond(c, x0, y0, w, h):
    """Les Marquises HENUA ENANA : l'illustration sous licence en filigrane —
    et l'archipel vectoriel maison en roue de secours si l'image manque."""
    img = _charger_marquises()
    if img:
    ...
    ILES = [
        # (nom, x, y, largeur mm, hauteur mm, étiquette à gauche ?)
        ("Eiao",      0.355, 0.86, 3.6, 1.8, True),
        ("Hatutu",    0.400, 0.905, 2.4, 1.2, False),
        ("Nuku Hiva", 0.420, 0.615, 7.0, 4.6, True),
        ("Ua Huka",   0.540, 0.60, 3.4, 1.9, False),
        ("Ua Pou",    0.445, 0.455, 3.8, 2.6, True),
        ("Hiva Oa",   0.600, 0.315, 6.2, 3.0, True),
        ("Tahuata",   0.585, 0.225, 2.8, 1.8, True),
        ("Mohotani",  0.655, 0.21, 2.0, 1.2, True),
        ("Fatu Hiva", 0.660, 0.08, 2.8, 2.0, False),
    ]
    for nomile, px, py, lw, lh, gauche in ILES:
        lw, lh = lw * 0.55, lh * 0.55
        cx, cy = x0 + w * px, y0 + h * (0.10 + py * 0.72)
        c.setFillColor(PALE)
        # forme organique : deux ellipses qui se chevauchent
        c.ellipse(cx - lw * mm / 2, cy - lh * mm / 2, cx + lw * mm / 2, cy + lh * mm / 2, stroke=0, fill=1)
        c.ellipse(cx - lw * mm * 0.28, cy - lh * mm * 0.15, cx + lw * mm * 0.55, cy + lh * mm * 0.62, stroke=0, fill=1)
    ...
    # les deux villes principales (points + noms)
    for ville, px, py in (("Taiohae", 0.432, 0.575), ("Atuona", 0.615, 0.30)):
        cx, cy = x0 + w * px, y0 + h * (0.10 + py * 0.72)
        c.setFillColor(colors.Color(0.62, 0.62, 0.62))
        c.circle(cx, cy, 0.45 * mm, stroke=0, fill=1)
        c.setFont(POLICE, 2.8)
        c.drawString(cx + 0.8 * mm, cy - 1.1, ville)
