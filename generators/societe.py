# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur SOCIÉTÉ (format A4)
...
def _charger_societe():
    """L'illustration des îles de la Société (licence à fournir par Maeva),
    pâlie en filigrane. Anti-panne : archipel vectoriel maison en secours."""
    global _SOCIETE_IMG
    if _SOCIETE_IMG is not None:
        return _SOCIETE_IMG
    try:
        from PIL import Image as _Image
        chemin = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "societe_archipel.png")
...
def _dessiner_fond(c, x0, y0, w, h):
    """La SOCIÉTÉ : l'illustration sous licence en filigrane —
    et l'archipel vectoriel maison en roue de secours si l'image manque."""
    img = _charger_societe()
    if img:
        ...
        return
    # ── repli : les VRAIES îles de la Société, dessinées maison ──
    # (la géographie est un fait du monde : positions et noms réels — des îles
    #  hautes pleines, et Tahiti avec sa presqu'île Tahiti Iti)
    ILES = [
        # (nom, x, y, largeur mm, hauteur mm, côté de l'étiquette : g/d/b/h)
        ("Maupiti",   0.295, 0.62, 1.8, 1.4, "b"),
        ("Bora Bora", 0.370, 0.685, 2.4, 1.8, "h"),
        ("Tahaa",     0.415, 0.60, 2.0, 1.6, "d"),
        ("Raiatea",   0.420, 0.505, 2.6, 2.2, "b"),
        ("Huahine",   0.495, 0.565, 2.2, 2.4, "d"),
        ("Maiao",     0.560, 0.32, 1.4, 1.1, "b"),
        ("Moorea",    0.625, 0.385, 2.6, 2.0, "h"),
        ("Tahiti",    0.710, 0.315, 4.6, 3.4, "b"),
    ]
    ...
    for nomile, px, py, lw, lh, cote in ILES:
        ...
        if nomile == "Tahiti":
            # la presqu'île Tahiti Iti, accrochée au sud-est
            c.ellipse(cx + lw * mm * 0.30, cy - lh * mm * 0.75,
                      cx + lw * mm * 0.85, cy - lh * mm * 0.18, stroke=0, fill=1)
