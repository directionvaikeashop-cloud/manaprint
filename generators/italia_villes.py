"""
🗺️ ITALIA VILLES — LE JUMEAU CARTE (02/08, sur le dessin de Maeva)
Chaque grande ville d'Italie porte son numéro dans un rond blanc ;
...
try:
    pdfmetrics.registerFont(TTFont("DJBOLDIV", "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"))
    _POLICE_ECO = "DJBOLDIV"
except Exception:
    pass
...
GRAINE = 984000                       # graine propre (982000 = QUINES 90)

RAINBOW = ["#E63946", "#F4A261", "#E9C46A", "#2A9D8F", "#264653",
           "#8E7DBE", "#D62828", "#457B9D", "#6A994E", "#BC6C25",
           "#7209B7", "#F72585"]

# ── contour de repli (inutilisé tant que la carte est là) ────────────────
COTE = [
...
COMMUNES = [
    ("Milan", 0.2365, 0.8262),
    ("Venise", 0.4558, 0.8274),
    ("Rome", 0.4231, 0.5143),
    ("Naples", 0.5596, 0.3881),
    ("Bari", 0.7846, 0.4286),
    ("Palerme", 0.4423, 0.1464),
]

# chaque commune tire dans SA plage : le carton fait le tour de 1 à 75
PLAGES_COMMUNES = [(1, 15), (16, 30), (31, 45), (46, 60), (46, 60), (61, 75)]
...
import os as _os
_IMAGE_ILE = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "italia_carte.png")
_RATIO_ILE = 900.0 / 970.0     # proportions du dessin de Maeva


def _ile(c, x0, y0, zx, zy, zw, zh, col):
    """La carte de Maeva, pâlie pour l'encre — repli : le contour au trait."""
...
def _dessiner_carte(c, x0, y0, nums, couleur_hex, serie, titre_jeu="", telephone="",
    ...
    tete = "ITALIA \u00b7 le tour des villes"
    if titre_jeu:
    ...
    if _sec and evenement_id:
        try:
            q = 14.0 * mm
            _sec.carton_qr(c, x0 + zx + zw * 0.17 - q / 2, y0 + zy + zh * 0.30 - q / 2,
...
if __name__ == "__main__":
    pdf = generer_pdf(nb_cartes=6, couleur=True, titre_jeu="ITALIA VILLES", telephone="89 22 23 05")
    with open("test_italia_villes.pdf", "wb") as f:
        f.write(pdf.read())
    print("ITALIA VILLES généré")
