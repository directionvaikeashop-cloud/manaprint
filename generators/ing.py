# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur ING (format A4)
12 cartes par feuille A4 (3 colonnes × 4 rangées).
Chaque carte : en-tête I | N | G puis grille complète 3×3 — 8 numéros,
la CASE CENTRALE du N restant vide (fidèle au modèle) :
  I = 16-30 (×3, croissants),  N = 31-45 (×2, DÉCROISSANTS — le plus grand
  en haut, la coquetterie du modèle !),  G = 46-60 (×3, croissants)
Le QR de vérification habite la case vide du centre — comme son jumeau INO.
...
def _dessiner_carte(c, x0, y0, cols_nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id="", jetons=False):
    ...
    signature = "ING"
    if titre_jeu and "ING" not in titre_jeu.strip().upper():
        signature += " · " + titre_jeu.strip()
    if telephone:
        signature += " · " + telephone
    c.setFillColor(col); c.setFont(POLICE, 3.8)
    if not jetons: c.drawCentredString(x0 + CARD_W / 2, hdr_bas - 2.4 * mm, signature[:60])

    # Pied de carte : « N° SERIE | 030001 »
    c.setStrokeColor(col); c.setLineWidth(0.4)
    if not jetons: c.line(x0 + 1.5 * mm, y0 + PIED_H, x0 + CARD_W - 1.5 * mm, y0 + PIED_H)
    if not jetons: c.line(x0 + CARD_W * 0.42, y0 + 0.8 * mm, x0 + CARD_W * 0.42, y0 + PIED_H - 0.6 * mm)
...
def generer_pdf(nb_cartes=12, serie_start=1, theme="", couleur=True,
                ...
                style="eco", evenement_id="", jetons=False):
    buf = io.BytesIO()
