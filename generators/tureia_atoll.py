...
try:
    pdfmetrics.registerFont(TTFont("DJBOLDTU", "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"))
    _POLICE_ECO = "DJBOLDTU"
except Exception:
    pass
...
GRAINE = 985000                       # graine propre (982000 = QUINES 90)
...
PLAGES_COMMUNES = [(1, 15), (1, 15), (16, 30), (16, 30),
                   (46, 60), (46, 60), (61, 75), (61, 75)]
...
def _dessiner_carte(c, x0, y0, nums, couleur_hex, serie, titre_jeu="", telephone="",
    ...
    while t > 5.4 and _lg(devise, "Helvetica-Bold", t) > place:
        t -= 0.2
    c.setFillColor(col); c.setFont("Helvetica-Bold", t)
    c.drawCentredString(x0 + CARD_W / 2, y0 + CARD_H - 6.0 * mm, devise)
    # le titre du client et son téléphone vivent sur LEUR ligne
    sous = ""
    if titre_jeu and titre_jeu.strip().upper() != "TUREIA":
        sous = titre_jeu.strip()[:30]
    if telephone:
        sous = (sous + "  " if sous else "") + str(telephone)[:16]
    if sous:
        c.setFont("Helvetica", 6.6)
        while len(sous) > 8 and _lg(sous, "Helvetica", 6.6) > place:
            sous = sous[:-1]
        c.drawCentredString(x0 + CARD_W / 2, y0 + CARD_H - 9.6 * mm, sous)
    c.setFont("Helvetica", 6.2)
    c.drawCentredString(x0 + CARD_W / 2, y0 + CARD_H - 12.8 * mm, "Carte N\u00b0 %05d" % serie)
