...
(ordre LIBRE — fidèle au modèle). La case du bas-milieu est libérée
pour le QR de vérification (décision Maeva).
LE TIRAGE LE PLUS EXPRESS DU CATALOGUE : 1-27 !
...
PAGE_W, PAGE_H = A4
# Les 3 familles de NEUF (fidèle au modèle)
COLONNES = [(1, 9), (10, 18), (19, 27)]
...
def _gen_carte(rng):
    """5 boules : 2 + 1 + 2 par famille de neuf, ordre LIBRE.
    Le milieu libère sa case du bas pour le QR (décision Maeva)."""
...
def _dessiner_carte(c, x0, y0, cols_nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id="", motif=""):
    ...
    # En-tête 2 lignes (fidèle au modèle POW) — le nom du jeu TOUJOURS visible
    ...
    # Les 5 boules — le milieu garde son numéro en haut, ordre libre
...
def generer_pdf(nb_cartes=12, serie_start=1, theme="", couleur=True,
    ...
    rng = random.Random(937600 + int(serie_start))
...
if __name__ == "__main__":
    ...
    with open("test_pow6.pdf", "wb") as f:
        f.write(pdf.read())
    print("POW 5 boules généré")
