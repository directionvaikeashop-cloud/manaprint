# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur TEA (format A4)
12 cartes par feuille A4 (3 colonnes × 4 rangées).
Chaque carte : en-tête T | E | A (bande à séparateurs verticaux, fidèle au modèle)
puis DEUX rangées de numéros — 2 numéros par lettre (6 numéros par carte) :
  T = 35-45,  E = 46-56,  A = 57-67   (plages officielles TUKEA, juin 2026)
...
PAGE_W, PAGE_H = A4
LETTRES = "TEA"
# (min, max) par lettre — plages officielles TUKEA
...
def _gen_carte(rng):
    """6 numéros : 2 par lettre, chacun dans sa plage, empilés triés."""
...
def _dessiner_carte(c, x0, y0, cols_nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id=""):
    ...
    signature = "TEA by TUKEA"
    if titre_jeu and titre_jeu.strip().upper() != "TEA":
    ...
    # Zone des numéros : 2 rangées, 2 numéros par lettre (QR dans la bande basse)
...
def generer_pdf(nb_cartes=12, serie_start=1, theme="", couleur=True,
    ...
    rng = random.Random(995000 + int(serie_start))
...
if __name__ == "__main__":
    ...
    with open("test_tea.pdf", "wb") as f:
        f.write(pdf.read())
    print("TEA généré")
