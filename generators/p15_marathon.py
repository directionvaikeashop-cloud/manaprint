...
def generer_pdf(nb_cartes=15, serie_start=1, theme="", couleur=True,
    ...
    for _ in range(nb_pages):
        ...
        if nom_evenement:
            ...
            #    ⚠️ MESURÉ sur la feuille : il restait 11,1 pt de blanc entre le bas du
            ...
            c.drawCentredString(PAGE_W / 2, PAGE_H - 7.6 * mm, nom_evenement)
        c.setFillColor(GRIS_CLAIR); c.setFont(POLICE, 6)
        if nom_evenement:
            c.drawRightString(PAGE_W - 6 * mm, PAGE_H - 7.6 * mm, "%03d" % no_page)
        else:
