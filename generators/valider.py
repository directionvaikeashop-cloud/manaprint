# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur VALIDER (format A4)
12 billets/page — 6 GRANDES BOULES à double cercle qui se chevauchent (façon
modèle), titre VALIDER gris en haut-gauche ; zones : 2×31-45 · 2×46-60 · 2×61-75.
...
NB_NUMS = 6
# chemin décodé sur le billet modèle (38/44/55/60/63/75) :
# milieu-gauche puis haut-centre = 2 triés dans 31-45 · haut-droit puis
# centre = 2 triés dans 46-60 · bas-centre puis bas-droit = 2 triés dans
# 61-75 (les tranches 1-15 et 16-30 sont absentes du modèle)
POSITIONS = [(0.17, 0.50), (0.42, 0.72), (0.82, 0.72), (0.55, 0.47), (0.42, 0.20), (0.80, 0.20)]
RAYON_BOULE_MM = 9.0
TAILLE_CHIFFRE = 32
...
def _dessiner_carte(c, x0, y0, nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id=""):
    ...
    # ── le titre VALIDER en gris, SOUS le numéro de série (demande Maeva) ──
    c.setFillColor(colors.Color(0.55, 0.55, 0.55)); c.setFont(POLICE, 9)
    c.drawString(x0 + 2.6 * mm, y0 + CARD_H - 12.8 * mm, "VALIDER")

    # en-tête : nom + signature + N° de carte
    titre = "VALIDER 6 boules"
    ...
    # les 6 GRANDES BOULES à double cercle (fond blanc : celles du dessous
    # passent derrière, comme les boules qui se chevauchent du modèle)
    r = RAYON_BOULE_MM * mm
    for i, (px, py) in enumerate(POSITIONS[:len(nums)]):
        ccx = x0 + CARD_W * px
        cy = y0 + CARD_H * py
        c.setFillColor(colors.white); c.setStrokeColor(colors.Color(0.55, 0.55, 0.55)); c.setLineWidth(0.7)
        c.ellipse(ccx - r, cy - r, ccx + r, cy + r, stroke=1, fill=1)
        c.setLineWidth(0.45)
        ri = r * 0.80
        c.ellipse(ccx - ri, cy - ri + 0.7 * mm, ccx + ri, cy + ri + 0.7 * mm, stroke=1, fill=0)
        if _sec:
    ...
    # QR de vérification (anti-duplication) — dans la POCHE BLANCHE à côté
    # du numéro central (entre la boule du centre et le bord droit)
    if _sec and evenement_id:
        try:
            _q = 8.5 * mm
            _sec.carton_qr(c, x0 + 47.0 * mm, y0 + 26.0 * mm, _q, evenement_id, serie)
        except Exception:
            pass
    ...
    for _ in range(nb_pages):
        ...
        for row in range(ROWS_PAGE):
            for col_i in range(COLS_PAGE):
                ...
                za = sorted(rng.sample(range(31, 46), 2))  # milieu-gauche puis haut-centre
                zb = sorted(rng.sample(range(46, 61), 2))  # haut-droit puis centre
                zc = sorted(rng.sample(range(61, 76), 2))  # bas-centre puis bas-droit
                nums = [za[0], za[1], zb[0], zb[1], zc[0], zc[1]]
