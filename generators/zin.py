# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur ZIN (format A4)
12 cartes par feuille A4 (3 colonnes × 4 rangées).
Chaque carte : en-tête Z | I | N puis grille complète 3×3 — 6 boules et
l'ESCALIER du modèle : trois familles de DOUZE, chacune sur 2 rangées :
  Z = 1-12 (rangées 1 et 3) · I = 13-24 (rangées 1 et 2)
  N = 25-36 (rangées 2 et 3), le tout trié vers le bas.
Trois cases vides en diagonale — celle du haut-droit accueille le QR.
Pied de carte : « N° SERIE | 030001 ». Tirage compact 1-36 !
...
PAGE_W, PAGE_H = A4
LETTRES = "ZIN"
# (min, max, rangées occupées) par lettre — l'escalier du modèle
COLONNES = [(1, 12, (0, 2)), (13, 24, (0, 1)), (25, 36, (1, 2))]
...
def _gen_carte(rng):
    """6 numéros : 2 par famille de douze, triés vers le bas."""
    return [sorted(rng.sample(range(pmin, pmax + 1), len(rangs)))
            for pmin, pmax, rangs in COLONNES]
...
def _dessiner_carte(c, x0, y0, cols_nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id=""):
    ...
    signature = "Le jeu ZIN pour 6 boules by TUKEA"
    if titre_jeu and "ZIN" not in titre_jeu.strip().upper():
    ...
    # Zone des numéros : grille complète 3×3, le N saute sa case centrale
    ...
    for ci, ((pmin, pmax, rangs), nums) in enumerate(zip(COLONNES, cols_nums)):
        cx = x0 + (ci + 0.5) * cell_w
    ...
    # QR de vérification — logé dans la case vide du HAUT-DROIT (l'escalier ZIN)
    if _sec and evenement_id:
        try:
            _q = min(row_h - 2.0 * mm, cell_w - 3 * mm, 13.0 * mm)
            _sec.carton_qr(c, x0 + 2.5 * cell_w - _q / 2,
                           z_top - 0.5 * row_h - _q / 2, _q, evenement_id, serie)
        except Exception:
            pass
...
def generer_pdf(nb_cartes=12, serie_start=1, theme="", couleur=True,
    ...
    rng = random.Random(936700 + int(serie_start))
