# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur BIO 8 BOULES (format A4)
12 cartes par feuille A4 (3 colonnes × 4 rangées).
Chaque carte : en-tête B | I | O puis grille complète 3×3 — 8 numéros triés
vers le bas, la CASE CENTRALE du I restant vide (décision Maeva, dans la
tradition INO 8 / ING) :
  B = 1-15 (×3),  I = 16-30 (×2, rangées 1 et 3),  O = 61-75 (×3)
Le QR de vérification habite la case vide du centre — la signature TUKEA.
...
PAGE_W, PAGE_H = A4
LETTRES = "BIO"
# (min, max, nombre) par lettre — le I saute sa case centrale
COLONNES = [(1, 15, 3), (16, 30, 2), (61, 75, 3)]
...
def _gen_carte(rng):
    """8 numéros : 3 pour I et O, 2 pour N (case centrale vide), triés."""
...
def _dessiner_carte(c, x0, y0, cols_nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id=""):
    ...
    signature = "BIO 8 boules by TUKEA"
    ...
    for ci, ((pmin, pmax, n), nums) in enumerate(zip(COLONNES, cols_nums)):
        cx = x0 + (ci + 0.5) * cell_w
        rangees = (0, 2) if n == 2 else (0, 1, 2)   # le N saute la rangée centrale
...
def generer_pdf(nb_cartes=12, serie_start=1, theme="", couleur=True,
    ...
    rng = random.Random(936100 + int(serie_start))
