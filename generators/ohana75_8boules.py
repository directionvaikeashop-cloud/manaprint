...
def _dessiner_carte(c, x0, y0, nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id="", idx_img=-1, myst_pos=None, myst_mnt=None):
    ...
    for i, (val, rond) in enumerate(nums):
        x = xs[i]
        if myst_pos is not None and i // 2 in myst_pos:   # 💰 colonne condamnée
            if i % 2 == 0:   # le rectangle se dessine UNE fois, CENTRÉ sur la paire
                _montant_rectangle(c, (xs[i] + xs[i + 1]) / 2, cy, 26 * mm, 16.5 * mm,
                                   myst_mnt[myst_pos.index(i // 2)], col, gris_ch)
            continue
...
def generer_pdf(nb_cartes=9, serie_start=1, theme="", couleur=True,
                ...
                style="eco", evenement_id="", smorfia=False, mystere=False, page_start=1):
    buf = io.BytesIO()
    ...
    for _ in range(nb_pages):
        ...
        for row in range(CARTES_PAGE):
            ...
            if mystere:   # 💰 LA colonne condamnée et SON montant
                mp, mv = _mystere_carte(serie, 4)
            coul = (couleur_perso if (couleur and couleur_perso)
                    else RAINBOW[(serie - 1) % len(RAINBOW)] if couleur else "#9A9A9A")
            _dessiner_carte(c, x0, y0, nums, coul, serie, titre_jeu, telephone, style=style, evenement_id=evenement_id, idx_img=idx_img, myst_pos=mp, myst_mnt=mv)
            serie += 1
            faites += 1
...
def generer_pdf_mystere(**kw):
    """💰 OHANA 75 \u00b7 8 boules MYSTÈRE — 2 coupes à MONTANTS remplacent 2 numéros."""
    kw["mystere"] = True
