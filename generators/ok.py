# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur OK (format A4)
...
def _charger_ok():
    """L'illustration de la main OK (licence Freepik, fournie par Maeva),
    pâlie en filigrane. Anti-panne : archipel vectoriel maison en secours."""
    global _OK_IMG
    if _OK_IMG is not None:
        return _OK_IMG
    try:
        from PIL import Image as _Image
        chemin = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "ok_main.png")
...
def _dessiner_fond(c, x0, y0, w, h):
    """OK : la main du fenua en filigrane —
    et l'archipel vectoriel maison en roue de secours si l'image manque."""
    img = _charger_ok()
    if img:
        from reportlab.lib.utils import ImageReader
        iw, ih = img.size
        zone_w, zone_h = w * 0.40, h * 0.46
        ratio = min(zone_w / iw, zone_h / ih)
        dw, dh = iw * ratio, ih * ratio
        c.drawImage(ImageReader(img), x0 + (w - dw) / 2, y0 + h * 0.22 + (zone_h - dh) / 2, dw, dh,
                    mask=[238, 255, 238, 255, 238, 255])
        return
    # ── repli : les lettres OK en grand contour, dessinées maison ──
    to = c.beginText()
    c.saveState()
    c.setStrokeColor(colors.Color(0.62, 0.62, 0.62)); c.setLineWidth(0.9)
    texte = "OK"
    c.setFont("Helvetica-Bold", 46)
    largeur = c.stringWidth(texte, "Helvetica-Bold", 46)
    to = c.beginText(x0 + (w - largeur) / 2, y0 + h * 0.38)
    to.setTextRenderMode(1)
    to.setFont("Helvetica-Bold", 46)
    to.textOut(texte)
    c.drawText(to)
    c.restoreState()
