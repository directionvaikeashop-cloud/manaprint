# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur PARATA (format A4)
6 billets-bandes/page — le nom PARATA en lettres évidées sur deux moitiés,
6 cases arrondies, chaque case tire dans SA plage : 1-15 · 60-75 · 76-90 · 16-30 · 31-45 · 46-59.
...
NB_NUMS = 6
# chaque case a SA plage, dans l'ordre du billet (gauche -> droite)
PLAGES = [(1, 15), (60, 75), (76, 90), (16, 30), (31, 45), (46, 59)]
# 3 cases par moitié : centres en fraction de largeur
POS_X = [0.115, 0.255, 0.395, 0.605, 0.745, 0.885]
TAILLE_CHIFFRE = 32
...
def _dessiner_carte(c, x0, y0, nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id=""):
    ...
    # le pointillé de partage entre les deux moitiés (comme le modèle)
    c.setStrokeColor(GRIS_CLAIR); c.setLineWidth(0.5); c.setDash(1, 2)
    c.line(x0 + CARD_W / 2, y0 + 2.5 * mm, x0 + CARD_W / 2, y0 + CARD_H - 2.5 * mm)
    c.setDash()

    # PA | RATA en lettres ÉVIDÉES (contour seul), une part sur chaque moitié
    band_y = y0 + CARD_H * 0.70
    t = c.beginText()
    t.setTextRenderMode(1)  # contour seul
    c.saveState()
    c.setStrokeColor(colors.Color(0.62, 0.62, 0.62)); c.setLineWidth(0.7)
    for texte, fx in (("PA", 0.25), ("RATA", 0.75)):
        c.setFont("Helvetica-Bold", 22)
        largeur = c.stringWidth(texte, "Helvetica-Bold", 22)
        to = c.beginText(x0 + CARD_W * fx - largeur / 2, band_y)
        to.setTextRenderMode(1)
        to.setFont("Helvetica-Bold", 22)
        to.textOut(texte)
        c.drawText(to)
    c.restoreState()

    # en-tête discret : nom + signature + N° de carte
    titre = "PARATA 6 plages"
    ...
    # les 6 cases arrondies, chaque numéro dans SA plage
    case_w, case_h = CARD_W * 0.125, CARD_H * 0.52
    case_y = y0 + CARD_H * 0.10
    for i, fx in enumerate(POS_X[:len(nums)]):
        ccx = x0 + CARD_W * fx
        c.setStrokeColor(colors.Color(0.62, 0.62, 0.62)); c.setLineWidth(0.7)
        c.roundRect(ccx - case_w / 2, case_y, case_w, case_h, 2.2 * mm, stroke=1, fill=0)
        cy = case_y + case_h / 2
        if _sec:
    ...
    # QR de vérification (anti-duplication) — bas-droit, sous la dernière case
...
def generer_pdf(nb_cartes=6, serie_start=1, theme="", couleur=True,
    ...
    rng = random.Random(952000 + int(serie_start))
