# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur OHANA 90 · 3 SÉRIES (format A4)
...
SERIES_3 = ["#F9A825", "#1E88E5", "#43A047"]  # jaune, bleu, vert
...
COLS_PAGE = 1
ROWS_PAGE = 3          # 3 grilles par feuille A4 (décision Maeva, 29/07)
MARGIN_X = 8 * mm
MARGIN_TOP = 8 * mm
MARGIN_BOT = 8 * mm
GUTTER = 7 * mm
...
CARD_H = (PAGE_H - MARGIN_TOP - MARGIN_BOT - (ROWS_PAGE - 1) * GUTTER) / ROWS_PAGE
HDR_H = 9 * mm
...
def _dessiner_carte(c, x0, y0, cols_paires, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id=""):
    ...
    ident = "OHANA 90 \u00b7 3 s\u00e9ries  \u2014  Carte N\u00b0 %06d" % serie
    ...
    # chiffres au calibre des cases : les 3 séries ont des cases plus courtes
    t_cercle = cell_h * 0.60
    t_petit = cell_h * 0.52
...
def generer_pdf(nb_cartes=3, serie_start=1, theme="", couleur=True,
    ...
    rng = random.Random(981000 + int(serie_start))
    ...
    for _ in range(nb_pages):
        ...
        c.setDash(3, 3)
        for k in range(1, ROWS_PAGE):
            y_coupe = MARGIN_BOT + k * CARD_H + (k - 1) * GUTTER + GUTTER / 2
            c.line(3 * mm, y_coupe, PAGE_W - 3 * mm, y_coupe)
        c.setDash()
        ...
        for row in range(ROWS_PAGE):
            for col_i in range(COLS_PAGE):
                ...
                coul = (couleur_perso if (couleur and couleur_perso)
                        else SERIES_3[pos] if couleur else "#9A9A9A")
