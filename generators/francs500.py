# -*- coding: utf-8 -*-
"""
MANAPRINT — Générateur 500 FRANCS (format A4)

💵 NÉ LE 13/08 (sceau Maeva) : LE BILLET DE CINQ CENTS FRANCS. Son
dessin — les poissons du lagon à gauche, le rameur à droite, les fleurs
en bandeau — occupe le carton, et les DIX numéros se rangent dans son
grand vide central.

RÈGLE : 10 numéros, DEUX par plage —
  1-15 · 16-30 · 31-45 · 46-60 · 61-75
Ils se lisent en DEUX RANGÉES de cinq : la première ligne au-dessus du
numéro de série, la seconde en dessous, comme sur le modèle de Maeva.
...
_RATIO_BILLET = 1.8605
ZONE = (0.163, 0.795, 0.255, 0.815)   # x0, x1, y0, y1
PLAGES500 = [(1, 15), (16, 30), (31, 45), (46, 60), (61, 75)]
import os as _os2
...
_IMAGE_BILLET = _choisir_image("billet500", _RATIO_BILLET)
...
_RATIO_BILLET_C = 2.0544
ZONE_C = (0.138, 0.749, 0.2, 0.855)
_IMAGE_BILLET_C = _choisir_image("billet500_c", _RATIO_BILLET_C)
...
try:
    ...
    _pm5.registerFont(_TF5("F500GROS",
        "/usr/share/fonts/truetype/dejavu/DejaVuSansCondensed-Bold.ttf"))
    _POLICE_GROS = "F500GROS"
except Exception:
    _POLICE_GROS = ""


def _gen_billet(rng):
    """💵 Les dix numéros : DEUX par plage, la haute et la basse.

    ⚠️ Sur le modèle de Maeva, la ligne du haut prend le PREMIER numéro
    de chaque plage et celle du bas le second — les deux lignes montent
    donc ensemble de gauche à droite.
    """
    haut, bas = [], []
    for pmin, pmax in PLAGES500:
...
def _dessiner_carte(c, x0, y0, nums, couleur_hex, serie, titre_jeu="", telephone="", style="eco", evenement_id=""):
    ...
    for etg, ys in ((haut, y_haut), (bas, y_bas)):
        for k, n in enumerate(etg):
            nx = zx + (k + 0.5) * place
            # ⚠️ l'étirement se fait par une TRANSFORMATION du canevas :
            # on garde l'origine du chiffre en place, on ne l'étire que
            # vers le haut. `saveState`/`restoreState` referment toujours.
            c.saveState()
            try:
                c.translate(nx, ys)
                c.scale(1.0, ETIRE)
    ...
    # 🎫 le numéro de série, dans son médaillon, entre les deux rangées
    my = zy + zh / 2 - 1.0 * mm
    mw, mh = 15.0 * mm, 4.6 * mm
    mx = zx + zw / 2 - mw / 2
    c.setFillColor(colors.white)
    c.setStrokeColor(col)
    c.setLineWidth(0.7)
    c.roundRect(mx, my - mh / 2, mw, mh, mh / 2, stroke=1, fill=1)
    c.setFillColor(col)
    c.setFont(POLICE, 6.2)
    c.drawCentredString(zx + zw / 2, my - 2.0, "%05d" % serie)
    ...
    if _sec and evenement_id:
        try:
            ...
            # ⚠️ en COULEUR le rameur descend plus bas : le QR se pose
            # sous le billet, dans la marge du carton, où rien ne vit.
            _q = 8.0 * mm
            if _est_couleur:
                _sec.carton_qr(c, x0 + CARD_W - _q - 1.0 * mm, y0 + 0.6 * mm,
